...
\begin{figure}
  \begin{center}
    ...
    \begin{axis}[
      ...
      % \pgfmathsetmacro{\b}{0.20}
      % \addplot[function={xblue}] ({-\a*sin(x)}, {\b*cos(x)+1.5});
      % \addplot[function={xred}] ({-sinh(x)}, {cosh(x)-0.5}) node[pos=0.75, above] {$e=2$};
      % \addplot[function={xred}] ({sinh(x)}, {-cosh(x)});
    \end{axis}
    \end{tikzpicture}
  \end{center}
...
% Cartesian coefficients A-F
% conic section fron 5 points
