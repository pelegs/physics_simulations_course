...
At any point in a Keplerian trajectory, the velocity of the satellite is always tangent to its trajectory. The angle between the direction of periapsis and the line connecting $F_{1}$ and the position of the satellite $r(t)$ at some given time $t$ is called the \textbf{true anomaly}, denoted as $\theta$.
...
The third constant is the \textbf{specific orbital energy} $\varepsilon$. It is defined by the \textbf{vis-viva equation}
\begin{equation}
  \varepsilon = \frac{v^{2}}{2} - \frac{\mu}{r},
  \label{eq:vis-viva}
\end{equation}
where $v$ is the speed (norm of the velocity) of the satellite at any time $t'$, and $r$ is the distance to the massive object at the same time $t'$. 

%The first is the \textbf{specific relative angular momentum} $h$, 
