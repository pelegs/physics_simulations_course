...
\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[thick, fill=black!50] (-1,0) rectangle (1,0.5);
			\draw[thick] (origin) -- (bob);
			\draw[thick, fill=xblue!25] (bob) circle (0.4);
			...
			\draw[vector, xblue] (bob) -- ($(bob)!0.3!(origin)$) node [midway, above right] {$\vec{T}$};
		\end{tikzpicture}
	\end{center}
	\caption{A simple pendulum with forces acting on the bob.}
