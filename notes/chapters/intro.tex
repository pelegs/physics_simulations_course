...
In the standard SI scheme, $F$ has units of \SI{}{\kilo\gram\meter\per\second}. We know that $m$ has units of \SI{}{\kilo\gram}, $v$ of \SI{}{meter\per\second} and $r$ units of \SI{}{\meter} - so both sides of the equation have the same units. However, nothing prevents us from measuring the rotation associated with $\frac{v^{2}}{r}$ directly using angular velocity - which can be defined as the amounth of degrees of rotation per second:
\begin{equation}
  F = m\omega^{2}r.
