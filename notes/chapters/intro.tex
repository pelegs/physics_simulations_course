...
\begin{example}{Taylor series of $\sin{x}$ and $\cos(x)$}{}
	...
	\begin{center}
		\tikz{
			...
			\node (-c) at (2,-2) {$-\cos(x)$};
			\node (s)  at (0,-2) {$\sin(x)$};
			\draw[->] (c)  --  (-s) node [midway, above] {$\od{}{x}$};
			\draw[->] (-s) --  (-c) node [midway, right] {$\od{}{x}$};
			\draw[->] (-c) --  (s)  node [midway, below] {$\od{}{x}$};
			\draw[->] (s)  --  (c)  node [midway, left]  {$\od{}{x}$};
		}
	\end{center}
...
Looking at the above results for $\EX{x},\ \sin(x)$ and $\cos(x)$, one can derive a nice connection: notice that 
\begin{equation}
