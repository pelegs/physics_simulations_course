...
\begin{figure}
  \begin{center}
    % TO BE DRAWN
  \end{center}
  \caption{Centripetal force. MORE TEXT.}
...
A much better approach is to measure the \textit{ratio} between the length of an arc created by our angle at a radius of \SI{1}{\meter} and the same radius itself (\autoref{fig:radians_measure}). This gives a unitless measure which we call \textit{radians}, and can be either ignored in the unit calculation (since it is unitless), or simply denoted \SI{}{\radian}.

\begin{figure}
  \begin{center}
    % TO BE DRAWN
  \end{center}
  \caption{Measuring angles using radians.}
...
Let's finish with some example correspondences between degrees and radians, which can be seen in \autoref{tab:degrees_to_radians}.
