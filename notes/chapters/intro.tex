...
The projection of a vector onto another vector gives rise to an important operation between two vectors: the \textbf{dot product}: given a vector $\vec{v}$, its projection on the vector $\vec{w}$ is
\begin{equation}
...
\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			\pgfmathsetmacro{\vx}{2.0}
			\pgfmathsetmacro{\vy}{1.5}
			\coordinate (v) at (\vx,\vy);
			\coordinate (w) at (3,0);
			\pgfmathsetmacro{\th}{atan(\vy/\vx)}
			\draw[thick, dashed] (2,1.5) -- (2,0);
			\draw[thick] (1.75,0) -- ++(0,0.25) -- ++(0.25,0);
			\draw[filledangle={xpurple}] (0,0) -- (0.5, 0) arc (0:\th:0.5) node [pos=0.35] {$\theta$};
			\draw[vector={xred}] (0,0) -- (v) node [midway, above left] {$\vec{v}$};
			\draw[vector={xblue}] (0,0) -- (w) node [midway, below] {$\vec{w}$};
		\end{tikzpicture}
	\end{center}
...
\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[vector={xred}] (0,0) -- (v) node [midway, above left] {$\vec{v}$};
			\draw[vector={xblue}] (0,0) -- (w) node [midway, below] {$\vec{w}$};
			\draw[vector={xpurple}] (w) -- (v) node [midway, right] {$\vec{c}$};
		\end{tikzpicture}
	\end{center}
...
Let us now explore some key properties of the dot product:
\begin{enumerate}
