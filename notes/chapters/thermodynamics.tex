...
\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			\begin{scope}[local bounding box=circ_aabb]
				\node[circle, draw=xblue, thick, fill=xblue!30, minimum size=1cm, inner sep=0pt, outer sep=0pt] at (0.5,0) {};
			\end{scope}
			\draw[xred] (circ_aabb.south west) rectangle (circ_aabb.north east);

			\begin{scope}[local bounding box=star1_aabb]
				\node[star, star points=6, draw=xgreen, thick, fill=xgreen!30, minimum size=1.5cm, inner sep=0pt, outer sep=0pt, rotate=30] at (-1.3,-0.7) {};
			\end{scope}
			\draw[xred] (star1_aabb.south west) rectangle (star1_aabb.north east);

			\begin{scope}[local bounding box=star2_aabb]
				\node[star, star points=6, draw=xpurple, thick, fill=xpurple!30, minimum size=1.5cm, inner sep=0pt, outer sep=0pt] at (-2.3,0.9) {};
			\end{scope}
			\draw[xred] (star2_aabb.south west) rectangle (star2_aabb.north east);

			\begin{scope}[local bounding box=poly_aabb]
				\draw[thick, xorange, fill=xorange!30] (-2.6,-1.0) -- ++(-0.25,0.6) -- ++(-0.5,-2.0) -- ++(0.5,0.8) -- cycle;
			\end{scope}
			\draw[xred] ($(poly_aabb.south west)+(-0.8pt,-0.8pt)$) rectangle ($(poly_aabb.north east)+(+0.8pt,+0.8pt)$);
		\end{tikzpicture}
	\end{center}
...
When two objects collide, their AABBs overlap (\autoref{fig:AABB_2}).

\begin{figure}
	\begin{center}
		\begin{tikzpicture}
			\tikzset{
				bsphere/.style n args={2}{circle, thick, draw=#1, fill=#1!30, minimum size=#2, inner sep=0pt, outer sep=0pt},
				sp1/.style={bsphere={xblue}{1.5cm}},
				sp2/.style={bsphere={xgreen}{1.25cm}},
			}
			\begin{scope}[local bounding box=circ1_aabb]
				\node[sp1] at (0,0) {};
			\end{scope}
			\draw[xred] (circ1_aabb.south west) rectangle (circ1_aabb.north east);
			\begin{scope}[local bounding box=circ2_aabb]
				\node[sp2] at (-2,1) {};
			\end{scope}
			\draw[xred] (circ2_aabb.south west) rectangle (circ2_aabb.north east);
