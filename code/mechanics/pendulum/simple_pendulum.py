...
def animate(frame):
    ...
    time_plt_theta.set_data(time_series[:frame], theta[:frame] / np.max(theta))
    time_plt_omega.set_data(time_series[:frame], omega[:frame] / np.max(omega))
    time_plt_alpha.set_data(
        time_series[:frame], alpha[:frame] / np.max(alpha[:-1])
    )
    return [
        rod,
        bob,
...
# Run animation
anim = FuncAnimation(
    fig, animate, frames=num_steps - 1, interval=20, blit=True
)
plt.show()
