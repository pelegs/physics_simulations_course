...
class QuadNode(object):
    ...
    def __iter__(self):
        """Iterates (lazily) over all the points located within a node & its
        children.
    ...
    def find(self, point):
        """Searches for the node that would contain the `Point` within the node
        & it's children.
    ...
    def all_points(self):
        """Returns a **list** of all the points located within a node & its
        children.
...
class QuadTree(object):
    ...
    def reset(self):
        self._root = self.node_class(
            self.center, self.width, self.height, capacity=self.capacity
        )
